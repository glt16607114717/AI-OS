...
| 入口 | 场景 | 工具执行域 |
|------|------|----------|
| `/v1/chat/completions` | 代理模式（TRAE/VSCode/Cursor） | 客户端执行（LS/Read/Grep 等） |
| `/api/workspace/chat` | 工作台模式（AIOS 自有界面） | 服务端执行 |

### 10.2 核心决策

**代理模式不注入服务端内置技能，只有工作台模式注入。**

原因：OpenAI 协议假设所有 `tool_calls` 由同一个执行者处理。代理模式下，客户端工具在客户端执行，服务端技能在服务端执行，两个执行域无法优雅协调——特别是当大模型在同一个响应里同时返回客户端工具和服务端技能的调用时，无法正确分流。

### 10.3 代码隔离原则

两个入口（`ProxyChatCompletions` / `WorkspaceChat`）**从第一行代码就完全分开**，没有共享的入口函数：

```
代理入口 ProxyChatCompletions         工作台入口 WorkspaceChat
        │                                     │
        ├─ 解析请求                           ├─ 解析请求
        ├─ 注入上帝指令+RAG                    ├─ 注入上帝指令+RAG
        ├─ 【不注入技能】                      ├─ 【注入服务端技能】
        ├─ 额度降级                           │
        ├─ 路由选择                           ├─ 路由选择
        │                                     │
        ▼                                     ▼
   proxyForward()                       agentLoop()
   （纯透传，不碰工具）                   （技能循环，服务端执行）
        │                                     │
        └─────────────┬─────────────────────┘
                      ▼
            ...
            - GetRouteByStrategy()
            - RecordStat()
            - buildFailoverAttempts()
```

**共用的是纯基础设施**（模型调用、路由选择、统计记录），这些函数对"代理/工作台"完全无感知。
**隔离的是业务逻辑**（工具注入、响应处理、循环编排），两个入口互不调用、互不影响。

历史教训：曾经共用一个 `handleChat` 入口函数，通过 `source` 参数区分场景，导致一次修改影响两个场景（如额度降级误触工作台、工具注入逻辑相互污染）。拆分后此类问题不再可能发生。

### 10.4 乱查库问题的解决

工作台模式下，AI 可能滥用数据库查询技能（如反复执行 `SELECT 1` 探活）。解决方式不是代码层拦截，而是**提示词工程**：

1. **技能描述用"触发条件"风格**（参考 TRAE 的 SKILL.md）：告诉 AI "什么情况下才调用"，而不是"你能干什么"
2. **每个数据库连接必须填描述**：告诉 AI 这个库里装的是什么，让它能判断"这个问题跟这个库有没有关系"
3. **暂不加调用次数限制**：复杂任务确实需要多次查询，硬限制会误伤。先靠提示词约束，观察实际效果

### 10.5 变更记录

| 日期 | 变更内容 |
|------|---------|
| 2026-06-21 | 代理模式移除内置技能注入；工作台保留并优化提示词；拆分 handleChat 为两个独立入口 |
