...
**任何代码变更必须经用户确认后才能执行，AI 没有自主决策权。**

- 改一行、改一个文件、改一个配置，都必须先向用户说明改什么、为什么改、影响范围，等用户明确同意后再执行
- 禁止批量修改多个文件后一次性提交，每个变更点都需要单独确认
- 如果用户说"继续"或"可以"，仅代表同意上一步提出的方案，不代表授权后续所有变更
- 如果不确定某项变更是否需要确认，默认按需要确认处理
