"""
AI-OS 本地 Agent（瘦身版）
仅保留语音助手 + 健康检查，所有 API 已迁移到 Go 后端。
端口 18732，与 Go 后端（18731）互不干扰。
"""
...
VERSION = "2.0.0-slim"
PORT = int(os.environ.get("AIOS_PORT", "18732"))
...
def load_build_info():
    ...
    try:
        ...
        return {"build_time": "unknown", "version": "2.0.0"}
...
app = FastAPI(title="AI-OS Local Agent", version=VERSION)
...
async def on_startup():
    global voice
    build_time = BUILD_INFO.get("build_time", "unknown")
    logger.info(f"AI-OS Local Agent v{VERSION} started (pid={os.getpid()}, port={PORT}, build_time={build_time})")
...
async def voice_api(req: VoiceActionRequest):
    ...
    if action == "voice_set_enabled":
        ...
        return {"ok": True}
    elif action == "voice_sync_commands":
        # 从 Go 后端同步指令到本地 voice_config.json
        remote_commands = payload.get("commands", [])
        from voice.config import load_config, save_config
        cfg = load_config()
        # 转换 Go 格式到本地格式
        local_cmds = []
        ...
        cfg["commands"] = local_cmds
        save_config(cfg)
        logger.info(f"Synced {len(local_cmds)} commands from remote")
        return {"ok": True, "count": len(local_cmds)}
    else:
